...
readlink 1.soft  # 查看里面的内容 -- 路径
```
unlink: remove directory entry

[link unlink实现mv](../src/file/17_link_myMv.c)

我们删除文件，某种意义上只是让文件具备被释放的条件(os择机释放，具体时间由系统内部调度算法决定)

unlink函数特征 : unlink使文件硬链接为0, 没有dentry对应，但该文件仍不会被释放,要等到所有打开文件的进程关闭该文件，系统才会挑时间将该文件释放掉

[unlink demo](../src/file/18_unlink_exe.c)
